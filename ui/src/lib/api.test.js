...
describe("buildQueryString", () => {
    ...
    it("puts email alert settings with same-origin credentials", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("/v1/settings/alerts/email", expect.objectContaining({
            method: "PUT",
            credentials: "same-origin",
            headers: expect.objectContaining({
                Accept: "application/json",
    ...
    it("posts test email endpoint", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("/v1/settings/alerts/email/test", expect.objectContaining({
            method: "POST",
            credentials: "same-origin",
        }));
    });
